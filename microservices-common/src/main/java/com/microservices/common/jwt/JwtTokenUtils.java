...
//                .setClaims(Rolmap)
    ...
    public static boolean checkJWT(String token) {
        try {
            final Claims claims = Jwts.parser().setSigningKey(APPSECRET_KEY).parseClaimsJws(replaceBearer(token)).getBody();
            return claims != null;
        } catch (Exception e) {
//            e.printStackTrace();
            return false;
    ...
    public static String getUsername(String token) {
        Claims claims = Jwts.parser().setSigningKey(APPSECRET_KEY).parseClaimsJws(replaceBearer(token)).getBody();
